    ...
    <view class="login-page">
      ...
      <view class="auth-card">
        ...
        <view class="form-intro">
          <text class="form-title">{{ mode === 'login' ? '继续使用媒合智联' : mode === 'register' ? '先把资料补全' : '重置登录密码' }}</text>
          <text class="form-subtitle">{{ mode === 'login' ? '登录后可以继续查看需求和对接进展' : mode === 'register' ? '这些信息会用于筛选服务方和社群' : `用绑定的${resetChannel === 'email' ? '邮箱' : '手机号'}验证身份，再设置新密码` }}</text>
        </view>
        ...
        <view v-if="mode === 'register'" class="field identity-field">
          <view class="field-label-row"><text class="field-label">融资身份</text><text class="field-hint">甲方和乙方填写不同档案</text></view>
          <view class="identity-options">
            <view class="identity-option" :class="{ active: form.organization_type === 'capital' }" @tap="form.organization_type = 'capital'">
              <text class="identity-option-title">甲方 / 资金方</text><text class="identity-option-desc">银行、基金、券商、FA 或产业资本</text>
            </view>
            <view class="identity-option" :class="{ active: form.organization_type === 'project' }" @tap="form.organization_type = 'project'">
              <text class="identity-option-title">乙方 / 项目企业</text><text class="identity-option-desc">企业或项目，有融资或合作需求</text>
            </view>
          </view>
        </view>

        <view v-if="mode === 'register'" class="field identity-field">
          <view class="field-label-row"><text class="field-label">平台使用方式</text><text class="field-hint">与融资身份独立，可后续调整</text></view>
          <view class="identity-options">
            <view class="identity-option" :class="{ active: form.workflow_role === 'demand_owner' }" @tap="form.workflow_role = 'demand_owner'">
              <text class="identity-option-title">发布需求</text><text class="identity-option-desc">让 AI 帮我整理并发布合作需求</text>
            </view>
            <view class="identity-option" :class="{ active: form.workflow_role === 'service_provider' }" @tap="form.workflow_role = 'service_provider'">
              <text class="identity-option-title">寻找项目</text><text class="identity-option-desc">从需求广场找机会并与需求 Agent 沟通</text>
            </view>
          </view>
        </view>
        ...
        <view v-if="mode === 'register'" class="field">
          ...
          <view class="verification-wrap">
            <input v-model="form.code" class="field-input verification-input" type="number" maxlength="6" placeholder="请输入6位验证码" confirm-type="next" />
        ...
        <template v-if="mode === 'register'">
          ...
          <view class="profile-grid">
            <view class="field compact-field">
              ...
              <input v-model="form.company" class="field-input" type="text" maxlength="40" placeholder="公司名称" />
            </view>
            <view class="field compact-field">
        ...
        <!-- #ifdef MP-WEIXIN -->
        <view class="wechat-divider"><view /><text>或使用微信</text><view /></view>
        <button class="wechat-btn" :disabled="submitting" @tap="doWechatLogin">微信登录</button>
        <!-- #endif -->
      </view>
      ...
      <view v-if="ENV.ALLOW_DEMO_LOGIN" class="demo-panel">
...
function validate() {
  ...
  if (mode.value === 'register') {
    ...
    if (!['demand_owner', 'service_provider'].includes(data.workflow_role)) return '请选择你的工作身份'
...
async function submit() {
  ...
  try {
    const payload = { ...form.value, ...consentPayload.value }
    if (mode.value === 'register') {
...
onLoad((query) => {
  const code = String(query?.invite_code || query?.inviteCode || '').trim().toUpperCase()
  if (!code) return
  mode.value = 'register'
  form.value.invite_code = code
