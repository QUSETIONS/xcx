...
describe('内测资料表单契约', () => {
  ...
  it('圈层补充输入保留可交互高度，补充区和匹配选项存在', () => {
    ...
    expect(source).toContain('class="status-actions"')
    expect(source).toContain('function enterPlatform()')
    expect(source).toContain("uni.switchTab({ url: '/pages/index/index' })")
    expect(source).toContain('function continueEditing()')
    expect(source).toContain('id="intake-actions"')
